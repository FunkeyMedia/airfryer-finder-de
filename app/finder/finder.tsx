...
export function Finder({ products }: { products: Product[] }) {
  ...
  if (done) return <section className="inner-section shell finder-results"><div className="results-head"><div><span className="eyebrow">Deine Auswertung</span><h2>Diese Airfryer passen am besten</h2><p>Der Matching-Wert ist eine Orientierung aus deinen Antworten und den verfügbaren Produktdaten – kein Labortest.</p></div><button onClick={reset} className="button secondary"><RotateCcw size={17}/> Neu starten</button></div><div className="match-grid">{results.map(({product,score,reasons},index)=><article className={`match-card ${index===0?"winner":""}`} key={product.asin}>{index===0&&<span className="winner-label">Beste Gesamtpassung</span>}<div className="match-image"><Image src={productImage(product)} alt={`${product.marke} ${product.modellcode}`} fill sizes="(max-width:700px) 90vw, 280px"/></div><div className="match-score"><strong>{score}%</strong><span>Match</span></div><span className="eyebrow">{index===1?"Günstige Alternative":index===2?"Komfort-Alternative":index===3?"Weitere starke Wahl":product.unterkategorie}</span><h3>{product.marke} {product.modellcode || product.titel.split(",")[0]}</h3><ul>{(reasons.length?reasons:["gute datenbasierte Gesamtpassung","Bewertungen wurden berücksichtigt"]).map(r=><li key={r}><Check size={16}/>{r}</li>)}</ul><div className="match-meta"><span>{product.kapazitaetL?`${product.kapazitaetL} L`:"Kapazität offen"}</span><span><Star size={15} fill="currentColor"/> {product.bewertung||"–"}</span><b>{formatPrice(product.preisEUR)}</b></div><Link href={`/airfryer/${productSlug(product)}`} className="button primary">Gerät ansehen <ArrowRight size={17}/></Link></article>)}</div><div className="share-box"><Share2/><div><b>Ergebnis merken</b><p>Du kannst diese Auswahl jederzeit über deinen Browser teilen oder als Favorit speichern.</p></div></div></section>;
